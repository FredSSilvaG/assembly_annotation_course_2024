...
# Load the annotation
annotation <- fread("/data/users/fsilvagutierrez/03_Semester/assembly_course_2024/Annotation/Rld-2/MAKER/final/filtered.genes.renamed.final.gff3", header = FALSE, sep = "\t")
bed_genes <- annotation %>%
    ...
    select(-V9)
#if you do not have .fai file then you can generate it with this: samtools faidx assembly.fasta

top20_scaff <- fread("/data/users/fsilvagutierrez/03_Semester/assembly_course_2024/Assembly/Flye/assembly.fasta.fai", header = FALSE, sep = "\t") %>%
    select(V1, V2) %>%
    arrange(desc(V2)) %>%
    head(20)
...
longest_proteins <- "/data/users/fsilvagutierrez/03_Semester/assembly_course_2024/Annotation/Rld-2/MAKER/final/assembly.all.maker.proteins.fasta.renamed.filtered.longest.fasta"
